...
class LeaveTypeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create leave type'))
        {
            ...
            $leavetype->is_attachment_required = $request->has('is_attachment_required') ? 1 : 0;
            $leavetype->min_advance_days     = $request->min_advance_days ?? 0;
            $leavetype->is_default           = 0;
    ...
    public function destroy(LeaveType $leavetype)
    {
        if(\Auth::user()->can('delete leave type'))
        {
            ...
            {
                return redirect()->back()->with('error', __('Default leave types cannot be deleted.'));
            }
