...
class PaySlipController extends Controller
{
    ...
    public function unpaysalary($id, $date)
    {
        // Only company owners may reverse payment
        ...
        if ($employeePayslip->status != 1) {
            return redirect()->route('payslip.index')->with('error', __('Payslip is already unpaid.'));
        }

        // Reverse the bank account debit that was applied during paysalary
        $account = Employee::find($id);
        Utility::bankAccountBalance($account->account, $employeePayslip->net_payble, 'credit');

        $employeePayslip->status = 0;
        $employeePayslip->save();

        return redirect()->route('payslip.index')->with('success', __('Payslip payment reversed successfully.'));
    }
