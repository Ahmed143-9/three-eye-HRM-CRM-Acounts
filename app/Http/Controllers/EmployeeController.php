...
use App\Models\EmergencyContactFile;
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create employee'))
        {
            ...
            if($request->has('emergency_contacts')) {
                $emergencyContacts = $request->emergency_contacts;
                ...
                foreach($emergencyContacts as $index => $contact) {
                    ...
                    if($index == 1 && empty($contact['full_name'])) {
                        continue;
                    }
                    
                    // Create emergency contact
                    $emergencyContact = EmployeeEmergencyContact::create([
...
//                        $path = $request->file('document')[$key]->storeAs('uploads/document/', $fileNameToStore);
    ...
    public function destroy($id)
    {
        ...
        if(Auth::user()->can('delete employee'))
        {
            ...
            // Clean up emergency contact files
    ...
    public function show($id)
    {
        ...
        if(\Auth::user()->can('view employee'))
        {
            try {
                ...
                $employee     = Employee::with(['emergencyContacts' => function($query) {
