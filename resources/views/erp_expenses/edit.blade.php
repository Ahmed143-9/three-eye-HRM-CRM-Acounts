{{ Form::model($expense, ['route' => ['erp-expenses.update', $type, $expense->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data']) }}
<div class="modal-body">
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('erp_expense_category_id', __('Category'), ['class' => 'form-label']) }}<span class="text-danger">*</span>
                {{ Form::select('erp_expense_category_id', $categories, null, ['class' => 'form-control select2', 'required' => 'required', 'id' => 'expense_category']) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
        ...
        @if($type == 'convenience')
            ...
            <div class="col-md-6 {{ $expense->transport_id ? '' : 'd-none' }}" id="transport_box">
                <div class="form-group">
        ...
        @if($type == 'purchase' || $type == 'convenience' || $type == 'utility' || $type == 'salary')
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    {{ Form::text('designation', $expense->employee->designation->name ?? '', ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'designation']) }}
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
                    {{ Form::label('department', __('Department'), ['class' => 'form-label']) }}
                    {{ Form::text('department', $expense->employee->department->name ?? '', ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'department']) }}
                </div>
            </div>
        @endif
        ...
        @if($type == 'salary')
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    {{ Form::number('net_salary', null, ['class' => 'form-control salary-calc', 'step' => '0.01', 'id' => 'net_salary']) }}
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    {{ Form::label('deduction_amount', __('Deduction Amount'), ['class' => 'form-label']) }}
                    {{ Form::number('deduction_amount', null, ['class' => 'form-control salary-calc', 'step' => '0.01', 'id' => 'deduction_amount']) }}
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    {{ Form::label('amount', __('Final Salary (Auto Calculated)'), ['class' => 'form-label']) }}
                    {{ Form::number('amount', null, ['class' => 'form-control', 'step' => '0.01', 'id' => 'final_salary', 'readonly' => 'readonly']) }}
                </div>
            </div>
            <div class="col-md-12">
                <div class="form-group">
        ...
        @if($type == 'utility')
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    {{ Form::number('amount', null, ['class' => 'form-control', 'step' => '0.01', 'required' => 'required']) }}
                </div>
            </div>
        @endif
                ...
                <div class="choose-files">
                    <label for="attachment">
                        ...
                        @if($expense->attachment)
                            <img id="blah" alt="your image" width="100" src="{{ asset($expense->attachment) }}" class="mt-2"/>
                        @else
                        ...
                        <table class="table mb-0" data-repeater-list="items" id="sortable-table">
                                ...
                                @if($expense->items->count() > 0)
                                    @foreach($expense->items as $item)
                                        <tr data-repeater-item>
                                            <td width="30%">
                                                {{ Form::text('product_name', $item->product_name, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Description')]) }}
                                            </td>
                                            <td width="15%">
                                                {{ Form::number('quantity', $item->quantity, ['class' => 'form-control quantity', 'required' => 'required', 'placeholder' => __('Qty'), 'step' => '0.01']) }}
                                            </td>
                                            <td width="15%">
                                                {{ Form::select('unit', $units, $item->unit->name ?? null, ['class' => 'form-control unit select2-tags', 'placeholder' => __('Unit')]) }}
                                            </td>
                                            <td width="15%">
                                                {{ Form::number('unit_price', $item->unit_price, ['class' => 'form-control unit_price', 'required' => 'required', 'placeholder' => __('Price'), 'step' => '0.01']) }}
                                            </td>
                                            <td width="20%">
                                                {{ Form::number('amount', $item->amount, ['class' => 'form-control amount', 'required' => 'required', 'placeholder' => __('Amount'), 'readonly' => 'readonly', 'step' => '0.01']) }}
                                            </td>
                                            <td width="5%">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="text-end"><strong id="total_amount">{{ $expense->amount }}</strong></td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
