...
@section('content')
    ...
    <div class="modal fade" id="markUnpaidModal" tabindex="-1" role="dialog" aria-labelledby="markUnpaidModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="markUnpaidModalLabel">{{ __('Reverse Payment') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('Close') }}"></button>
                </div>
                <div class="modal-body">
                    <p class="mb-0">{{ __('Are you sure you want to mark this payslip as unpaid?') }}</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <a href="#" id="markUnpaidConfirmBtn" class="btn btn-warning btn-sm">{{ __('Mark as Unpaid') }}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="deletePayslipModal" tabindex="-1" role="dialog" aria-labelledby="deletePayslipModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deletePayslipModalLabel">{{ __('Delete Payslip') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('Close') }}"></button>
                </div>
                <div class="modal-body">
                    <p class="mb-0">{{ __('Are you sure you want to delete this payslip? ') }}</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <button type="button" id="deletePayslipConfirmBtn" class="btn btn-danger btn-sm">{{ __('Delete') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@push('script-page')
    <script>
        $(document).ready(function() {
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            $.each(data, function(indexInArray, valueOfElement) {
                                ...
                                var markUnpaid = '';
                                @if (\Auth::user()->type == 'company')
                                    if (valueOfElement[6] == 'Paid' && valueOfElement[7] != 0) {
                                        var unpayUrl = '{{ url('payslip/unpaysalary/') }}/' + id + '/' + datePicker;
                                        markUnpaid = '<a href="#" ' +
                                            'class="btn-sm btn btn-warning me-1 mark-unpaid-btn" ' +
                                            'data-unpay-url="' + unpayUrl + '" ' +
                                            'data-bs-toggle="tooltip" title="{{ __('Mark as Unpaid') }}">' +
                                            '<i class="ti ti-rotate-clockwise-2"></i></a>';
                                    }
                                @endif
                                ...
                                tr +=
                                    ...
                                    '<td>' + status + '</td>' +
                                    '<td>' + payslip + clickToPaid + markUnpaid + edit + deleted + '</td>' +
                                    '</tr>';
                            });
                        } else {
            ...
            @if (\Auth::user()->type == 'company')
            $(document).on('click', '.mark-unpaid-btn', function() {
                var url = $(this).data('unpay-url');
                $('#markUnpaidConfirmBtn').attr('href', url);
                $('#markUnpaidModal').modal('show');
            });
            @endif

            var deletePayslipUrl = '';
            $(document).on("click", ".payslip_delete", function() {
                deletePayslipUrl = $(this).data('url');
                $('#deletePayslipModal').modal('show');
            });

            $(document).on("click", "#deletePayslipConfirmBtn", function() {
                $('#deletePayslipModal').modal('hide');
                $.ajax({
                    type: "GET",
                    url: deletePayslipUrl,
                    dataType: "JSON",
                    success: function(data) {
